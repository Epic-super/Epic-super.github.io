(function(){
  ...
  function appliedTheme(t){ return (t==='dark') ? 'dark' : (t==='light' ? 'light' : (osDark() ? 'dark' : 'light')); }
  function applyTheme(t){
    themeMode = validTheme(t);
    var act = appliedTheme(themeMode);
    document.documentElement.setAttribute("data-theme", act);
    updateThemeIcon(themeMode);
    var nf=document.getElementById('noclockFrame');
    if(nf && nf.contentWindow){ try{ nf.contentWindow.postMessage({type:'noclock-theme',theme:act},'*'); }catch(e){} }
  }
  var osMQ = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)');
  function onOSChange(){ if(themeMode==="auto") applyTheme("auto"); }
  if(osMQ && osMQ.addEventListener){ try{ osMQ.addEventListener('change', onOSChange); }catch(e){} }
  function updateThemeIcon(m){
    ...
    var auto='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 3v18"/></svg>';
    var el=$("#btnTheme");
    if(!el) return;
    el.innerHTML = (m==="dark") ? moon : ((m==="light") ? sun : auto);
    el.title = (m==="dark") ? "深色 · 点击切换" : ((m==="light") ? "浅色 · 点击切换" : "跟随系统 · 点击切换");
  }
  ...
  function validTheme(t){ return (t==="dark"||t==="light"||t==="auto") ? t : "auto"; }
  applyTheme(validTheme(store.get('theme') || store.getLegacy('wb_hub_theme')));
  $("#btnTheme").onclick=function(){
    var next = (themeMode==="dark") ? "light" : ((themeMode==="light") ? "auto" : "dark");
