...
export function initGamesShelf() {
  ...
  function lookupGt(g, list){if(!list||!list.length) return null;
    ...
    return null;}
  // 注意：外层 try/catch 只挡得住 fetch 的**同步**抛错（如 URL 非法）；
  // 网络失败返回的是 rejected promise，必须由 .catch 兜住，否则 init 里的
  // `loadGtGames().then(renderGames)` 会变成 unhandled rejection（每 60s 一次，
  // 污染 errorlog，且在 Node/SSR 类环境下直接崩溃进程）。
  // 游戏时长只是增强信息，拉取失败必须静默，绝不许影响游戏架渲染。
  function loadGtGames(){try{return fetch(GT_API,{cache:"no-store"}).then(function(r){return r&&r.ok?r.json():null;}).then(function(d){__gtGames=(d&&d.games)?d.games:[];}).catch(function(){});}
  ...
  function init(){
    ...
    // 必须走 window. 前缀：本文件是 ESM 模块，模块作用域里的裸标识符解析不到
    // window 上的属性（231 行是 window.importScannedGames=...）。
    // 旧写法裸用 importScannedGames → ReferenceError → 中断 initGamesShelf →
    // 在 boot() 里会连带截断后续全部桥接（全站点击失效的元凶之一）。
    var imp=$("#btnImportLib"); if(imp) imp.onclick=window.importScannedGames;
    renderGames();
    // 拉取游戏时长数据（首次即拉，失败静默回退到无时长渲染）
    // .catch 保底：renderGames 自身已被 viewGuard 包过，这里再兜一层，杜绝定时轮询里的未捕获拒绝
    loadGtGames().then(renderGames).catch(function(){});
    setInterval(function(){ loadGtGames().then(renderGames).catch(function(){}); }, 60000);  // 进行中接近实时
