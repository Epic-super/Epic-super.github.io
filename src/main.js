...
import '../lib/wb-elements.js'; // 可复用 Web Components（<wb-ring> 等）；先于业务模块注册
...
// A 方向 · 移动优先交互（下拉刷新/双击缩放/切Tab回顶，纯触屏增强）
import { initMobileInteractions } from './mobile-interactions.js';
// 左右 HUD 仪表舱（未来感科技侧栏，超宽屏读真实数据）
import { initHudRail } from './hud-rail.js';
...
function boot() {
  ...
  step('mobileInteractions', initMobileInteractions); // A 方向·移动优先：下拉刷新/双击缩放/切Tab回顶（纯触屏增强，桌面跳过）
  step('hudRail', initHudRail); // 左右 HUD 仪表舱：时钟/待办计数/版本号 数据刷新（超宽屏，读真实数据）
