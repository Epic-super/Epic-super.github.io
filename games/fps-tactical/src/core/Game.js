...
export class Game {
  constructor() {
    ...
    this.isMobile = window.matchMedia('(hover: none), (pointer: coarse), (max-width: 760px)').matches;
  }
  ...
  spawnEnemies() {
    ...
    spawnPoints.forEach(pos => {
      ...
      // Random initial state: 70% hold (dormant), 30% patrol
      if (Math.random() < 0.7) {
        enemy.setState('hold');
      } else {
  ...
  getTextState() {
    const weapon = this.player ? this.player.getCurrentWeapon() : null;
    return {
      mode: this.gameState,
      ...
      controls: this.isMobile ? 'touch joystick, swipe look, fire/ads/reload/run/jump buttons' : 'keyboard and mouse',
      coordinates: 'x/z are ground plane coordinates; y is height'
    };
  }
}
