...
export class Enemy {
  constructor(scene, physicsManager, audioManager, spawnPosition, assetLoader = null) {
    ...
    // AI State Machine
    this.state = 'hold'; // hold, patrol, alert, chase, shoot, reload, retreat
    this.stateTimer = 0;
    this.targetPosition = null;
    this.coverPosition = null;

    // Detection & alert behavior
    this.alertRadius = 12; // Player must be within this to trigger alert
    this.reactionDelay = 0.35; // Seconds before switching from hold -> alert
    this.alertCooldown = 0; // Prevents re-alerting every frame
    this.alertBroadcastCooldown = 0;
    this.hasLineOfSightToPlayer = false;

    // Combat - TACTICAL
    this.detectionRange = 20; // Patrol chase range
    this.shootRange = 18; // Combat range
    this.accuracy = 0.55;
    this.fireRate = 0.6;
    this.lastShotTime = 0;
    this.currentAmmo = 30;
  ...
  updateAI(delta, time, player, allEnemies) {
    ...
    this.alertCooldown = Math.max(0, this.alertCooldown - delta);
    this.alertBroadcastCooldown = Math.max(0, this.alertBroadcastCooldown - delta);

    const distanceToPlayer = this.position.distanceTo(player.position);
    this.hasLineOfSightToPlayer = this.hasLineOfSight(player.position);

    // Debug log occasionally
    if (Math.random() < 0.01) {
      console.log(`🤖 Enemy AI - State: ${this.state}, Distance: ${distanceToPlayer.toFixed(1)}, CanSee: ${this.hasLineOfSightToPlayer}`);
    }

    // State transitions
    switch (this.state) {
      case 'hold':
        // Stay still until player gets close enough
        if (distanceToPlayer < this.alertRadius && this.hasLineOfSightToPlayer) {
          if (this.alertCooldown <= 0) {
            this.alertCooldown = this.reactionDelay;
            console.log('👀 Enemy is alerting! Switching to alert in', this.reactionDelay.toFixed(2), 's');
            this.setState('alert');
          }
        }
        break;
      ...
      case 'alert':
        // Just spotted player -短暂停顿后转入 chase
        if (this.stateTimer >= this.reactionDelay) {
          console.log('🎯 Enemy engaging! Switching to chase');
          this.broadcastAlert(allEnemies);
          this.setState('chase');
        }
        break;

      case 'patrol':
        if (this.hasLineOfSightToPlayer && distanceToPlayer < this.alertRadius) {
      ...
      case 'chase':
        if (!this.hasLineOfSightToPlayer || distanceToPlayer > this.alertRadius * 2.0) {
          // Lost player - 回到 patrol 或 hold
          if (Math.random() < 0.5) {
            this.setState('patrol');
          } else {
            this.setState('hold');
          }
        } else if (distanceToPlayer < this.shootRange && this.hasLineOfSightToPlayer) {
      ...
      case 'shoot':
        ...
        } else if (!this.hasLineOfSightToPlayer || distanceToPlayer > this.shootRange * 1.2) {
          this.setState('chase');
        } else {
  ...
  broadcastAlert(allEnemies) {
    if (this.alertBroadcastCooldown > 0) return;
    this.alertBroadcastCooldown = 1.0; // Only broadcast every 1s

    const alertRange = 35; // Enemies within this range get alerted
    let alertedCount = 0;
    ...
    allEnemies.forEach(other => {
      ...
      if (dist < alertRange) {
        // Only alert enemies who are in hold or patrol
        if ((other.state === 'hold' || other.state === 'patrol' || other.state === 'idle') && other.alertCooldown <= 0) {
          other.alertCooldown = other.reactionDelay * (0.8 + Math.random() * 0.4); // Slight variation
    ...
    if (alertedCount > 0) {
      console.log(`📢 ${this.state === 'alert' ? 'Enemy' : 'Enemy'} alerted ${alertedCount} nearby allies!`);
    }
  }
  ...
  playStateAnimation(state) {
    ...
    switch(state) {
      ...
      case 'alert':
        // Brief alert pose - use idle or a subtle animation
        if (this.animations.idle) {
          this.animations.idle.reset().fadeIn(0.1).play();
        }
        break;
  ...
  updateMovement(delta) {
    // In hold state, enemies stay completely still
    if (this.state === 'hold') {
      this.velocity.x *= 0.8;
      this.velocity.z *= 0.8;
