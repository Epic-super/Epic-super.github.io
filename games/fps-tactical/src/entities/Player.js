...
export class Player {
  ...
  handleInput(delta) {
    ...
    const lookSensitivity = input.isTouchEnabled() ? this.mouseSensitivity * 1.35 : this.mouseSensitivity;
    this.yaw -= mouseMovement.x * lookSensitivity;
    this.pitch -= mouseMovement.y * lookSensitivity;
  ...
  updateMovement(delta) {
    ...
    const touchMove = input.getMovementVector();
    if (touchMove.y < -0.08) moveDir.add(forward.clone().multiplyScalar(-touchMove.y));
    if (touchMove.y > 0.08) moveDir.sub(forward.clone().multiplyScalar(touchMove.y));
    if (touchMove.x > 0.08) moveDir.add(right.clone().multiplyScalar(touchMove.x));
    if (touchMove.x < -0.08) moveDir.sub(right.clone().multiplyScalar(-touchMove.x));
