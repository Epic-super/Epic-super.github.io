...
(function(){
  ...
  function loadSettingsIntoForm(){
    ...
    // 默认音色不铺满输入框（留空即默认），自定义音色才回显
    var defV = (llm().defaultVoice || "");
    el.setVoice.value = (!tts.voice || tts.voice === defV) ? "" : tts.voice;
    updateCoachUI();
  }
  ...
  function saveLlm(){
    ...
    var voiceVal = el.setVoice.value.trim();
    next.tts = { enabled: el.setTtsOn.checked, speakJa: el.setSpeakJa.checked, voice: voiceVal || (llm().defaultVoice || "") };
    var c = llm().setConfig(next);
    el.setKey.value = "";
    state._ttsHinted = false; // 设置变过 → 允许再次提示语音状态
    updateCoachUI();
    var tip = (c.tts && c.tts.enabled && c.provider === "stepfun") ? "" : (llm().ttsHint ? llm().ttsHint() : "");
    setSettingsStatus((c.endpoint && c.model) ? ("已保存 · " + c.model + (tip ? " ｜ " + tip : "")) : "未配置");
  }
  function testLlm(){
  ...
  function sendChat(){
    ...
    llm().chat(text)
      .then(function(reply){
        ...
        if(llm().canSpeak()){
          var tts = c.tts || {};
          var coachEnglish = c.coachMode === 'english';
          // English Coach 直接读英文原文（不日译）；其余按「说日语」开关决定是否先日译
          var p = tts.speakJa && !coachEnglish
            ? llm().translateToJa(reply).then(function(ja){ return llm().speak(ja); })
            : llm().speak(reply);
          p.then(playAudio).catch(function(e){ showBubble('语音合成失败：' + ((e && e.message) || e)); });
        } else if(c.provider === "stepfun" && !state._ttsHinted){
          // 已配 StepFun 却没声音：给一次明确原因（不反复打扰）
          var hint = llm().ttsHint ? llm().ttsHint() : "";
          if(hint){ state._ttsHinted = true; showBubble(hint); }
        }
      })
  ...
  function playAudio(blob){
    ...
    el.audio.play().catch(function(){
      if(!state._playWarned){ state._playWarned = true; showBubble('浏览器拦截了自动播放：点一下页面后重试'); }
    });
  }
  ...
  function connect(){
    ...
    ensureEngine().then(function(ok){
      ...
      console.error("[amadeus] Live2D 启动失败，已降级为 SVG 陪伴模式：", err);
      var renderFail = err && err.message && /unsafe-eval|Application|renderer|systemCheck|WebGL|eval/i.test(err.message);
      el.boot.hidden = true;
      el.avatarWrap.hidden = false;
      widget.classList.add("connected");
      el.chat.hidden = false;
      showBubble(renderFail
        ? "Live2D 渲染初始化失败（多为浏览器/CSP 禁用 unsafe-eval 导致），已降级为 SVG 陪伴模式"
        : "Live2D 引擎不可用，已降级为 SVG 陪伴模式");
    }).then(function(){
      state.loading = false;
    });
  }
  ...
  function build(){
    ...
    widget.innerHTML =
      ...
      '<div class="ama-settings" id="amaSettings" hidden>'+
        ...
        '<div class="ama-set-row"><label>语音回复</label><label class="ama-check"><input type="checkbox" id="amaTtsOn"> 合成朗读（红莉栖声线）</label></div>'+
        '<div class="ama-set-row"><label>说日语</label><label class="ama-check"><input type="checkbox" id="amaSpeakJa"> 语音用日语</label></div>'+
        '<div class="ama-set-row"><label>TTS 音色</label><input id="amaSetVoice" placeholder="voice-tone-…（留空=默认红莉栖音色）" spellcheck="false"></div>'+
        '<div class="ama-set-hint">English Coach 模式按英文朗读，其余按回复语种自动切换（中 / 日 / 英）。</div>'+
    ...
    el.setVoice = widget.querySelector("#amaSetVoice");
  ...
  function init(){
    ...
    bindNav();
    watchLena();
    // 暴露打开钩子，供 pet-lazy 懒加载完成后直接调用（不依赖补派 click 与角色栏收起竞态）
    window.Amadeus = { open: togglePet, toggle: togglePet, isOpen: function(){ return state.open; } };
  }
