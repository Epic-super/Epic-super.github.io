...
(function(){
  ...
  var PRESETS = {
    ...
    // 音色用女生音色 linjiajiejie（文档示例），适配红莉栖。
    stepfun:  { provider:"stepfun", endpoint:"https://api.stepfun.com/step_plan/v1", model:"step-3.7-flash", tts:{ enabled:true, speakJa:true, model:"stepaudio-2.5-tts", voice:"linjiajiejie" } }
  };
  ...
  var api = {
    ...
    clearConfig: function(){
      cfg = { provider:"", endpoint:"", model:"", apiKey:"", coachMode:"normal", tts:{ enabled:false, speakJa:false, model:"step-tts-2", voice:"bleu" } };
  ...
  api.speak = async function(text){
    if(!api.canSpeak()) throw new Error("未启用 Step 语音或未配置");
    var body = { model: cfg.tts.model || "step-tts-2", input: String(text||""), voice: cfg.tts.voice || "bleu", response_format:"mp3" };
